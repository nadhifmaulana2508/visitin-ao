...
class ProspectController
{
    ...
    private function ensureColumnExists(string $table, string $column, string $definition): void
    {
        ...
        if ((int) $stmt->fetchColumn() === 0) {
            try {
                ...
                sendResponse(500, 'Struktur kolom pipeline belum tersedia dan gagal disiapkan', [
                    'error' => $e->getMessage(),
                ]);
            }
        }
    ...
    private function hasPipelineColumn(string $column): bool
    {
        static $columns = null;

        if ($columns === null) {
            try {
                $stmt = $this->db->query("SHOW COLUMNS FROM `prospect_credit_pipelines`");
                $columns = array_column($stmt->fetchAll(), 'Field');
            } catch (Throwable $e) {
                $columns = [];
            }
    ...
    private function decodeBase64Payload(string $base64): string
    {
    ...
    /**
     * # COMPONENT: Parser ringkasan IDEP untuk membantu review awal SLIK.
     * Hanya metadata risiko yang disimpan; isi mentah IDEP tidak disimpan ke database.
     */
    private function analyzeIdepContent(string $contents, int $requestedLoanAmount = 0): array
    {
        $contents = preg_replace('/^\xEF\xBB\xBF/', '', $contents) ?? $contents;
        // # COMPONENT: Beberapa export IDEP membawa karakter non-UTF-8; buang karakter rusak tanpa mengubah struktur JSON.
        if (function_exists('iconv')) {
            $normalizedContents = iconv('UTF-8', 'UTF-8//IGNORE', $contents);
            if ($normalizedContents !== false) {
                $contents = $normalizedContents;
            }
        }
        try {
            $report = json_decode(trim($contents), true, 512, JSON_THROW_ON_ERROR);
        } catch (JsonException $e) {
            sendResponse(400, 'Format file IDEP tidak valid. Gunakan file JSON IDEP berekstensi TXT.', null);
        }

        $individual = is_array($report['individual'] ?? null) ? $report['individual'] : [];
        $debtorData = is_array($individual['dataPokokDebitur'][0] ?? null) ? $individual['dataPokokDebitur'][0] : [];
        $summary = is_array($individual['ringkasanFasilitas'] ?? null) ? $individual['ringkasanFasilitas'] : [];
        $facilitiesRoot = is_array($individual['fasilitas'] ?? null) ? $individual['fasilitas'] : [];
        $facilities = [];
        foreach (['kreditPembiayan', 'lc', 'garansiYgDiberikan', 'fasilitasLain'] as $group) {
            if (!empty($facilitiesRoot[$group]) && is_array($facilitiesRoot[$group])) {
                foreach ($facilitiesRoot[$group] as $facility) {
        ...
        $toNumber = static function ($value): int {
        ...
        $qualityValues = [];
        $summaryQuality = $toNumber($summary['kualitasTerburuk'] ?? 0);
        if ($summaryQuality > 0) {
            $qualityValues[] = $summaryQuality;
        }
        $maxOverdueDays = 0;
        ...
        foreach ($facilities as $facility) {
            $quality = $toNumber($facility['kualitas'] ?? 0);
            if ($quality > 0) {
                $qualityValues[] = $quality;
                if (isset($qualityDistribution[$quality])) {
                    $qualityDistribution[$quality]++;
                }
            }
            $outstanding = $toNumber($facility['bakiDebet'] ?? 0);
            $totalOutstanding += $outstanding;
            if ($outstanding > 0) {
                $activeFacilities++;
            }
            $maxOverdueDays = max($maxOverdueDays, $toNumber($facility['jumlahHariTunggakan'] ?? 0));
            $totalPrincipalArrears += $toNumber($facility['tunggakanPokok'] ?? 0);
            $totalInterestArrears += $toNumber($facility['tunggakanBunga'] ?? 0);
            $totalPenalty += $toNumber($facility['denda'] ?? 0);
        }

        $worstQuality = $qualityValues ? max($qualityValues) : null;
        $effectiveLimit = $toNumber($summary['plafonEfektifTotal'] ?? 0);
        if ($totalOutstanding === 0) {
            $totalOutstanding = $toNumber($summary['bakiDebetTotal'] ?? 0);
        }
        $totalArrears = $totalPrincipalArrears + $totalInterestArrears + $totalPenalty;
        $utilizationPercent = $effectiveLimit > 0
            ? round(($totalOutstanding / $effectiveLimit) * 100, 2)
            : null;
        $exposureAfterRequest = $totalOutstanding + max(0, $requestedLoanAmount);
        $lenderCount = $toNumber($summary['krediturBankUmum'] ?? 0)
            + $toNumber($summary['krediturBPR/S'] ?? 0)
            + $toNumber($summary['krediturLp'] ?? 0)
            + $toNumber($summary['krediturLainnya'] ?? 0);
        ...
        $hasGoodQuality = count($facilities) > 0 && $worstQuality === 1 && $maxOverdueDays === 0 && $totalArrears === 0;
        $recommendationCode = count($facilities) === 0 || $worstQuality === null
            ? 'DATA_TIDAK_LENGKAP'
            : ($hasGoodQuality ? 'BAGUS' : ($worstQuality >= 3 || $totalArrears > 0 ? 'TIDAK_BAGUS' : 'PERLU_REVIEW'));
        $recommendationLabel = [
            'BAGUS' => 'SLIK baik',
            'PERLU_REVIEW' => 'Perlu review',
            'TIDAK_BAGUS' => 'Risiko terdeteksi',
            'DATA_TIDAK_LENGKAP' => 'Data belum lengkap',
        ][$recommendationCode];

        return [
            'schema_version' => 1,
            'source' => 'IDEP',
            'report_position' => (string) ($individual['posisiDataTerakhir'] ?? ''),
            'job' => (string) ($debtorData['pekerjaanKet'] ?? $debtorData['pekerjaan'] ?? ''),
            'business_field' => (string) ($debtorData['bidangUsahaKet'] ?? $debtorData['bidangUsaha'] ?? ''),
            'workplace' => (string) ($debtorData['tempatBekerja'] ?? ''),
            'job_assessment_code' => ($debtorData['pekerjaanKet'] ?? $debtorData['pekerjaan'] ?? '') !== ''
                || ($debtorData['bidangUsahaKet'] ?? $debtorData['bidangUsaha'] ?? '') !== ''
                || ($debtorData['tempatBekerja'] ?? '') !== ''
                ? 'PERLU_VERIFIKASI'
                : 'DATA_TIDAK_LENGKAP',
            'job_assessment_label' => (($debtorData['pekerjaanKet'] ?? $debtorData['pekerjaan'] ?? '') !== ''
                || ($debtorData['bidangUsahaKet'] ?? $debtorData['bidangUsaha'] ?? '') !== ''
                || ($debtorData['tempatBekerja'] ?? '') !== '')
                ? 'Perlu verifikasi lapangan'
                : 'Data pekerjaan belum tersedia',
            'job_assessment_note' => 'IDEP hanya menunjukkan data pekerjaan/bidang usaha yang tercatat. Validasi omzet, arus kas, tempat usaha, dan bukti pendukung tetap diperlukan.',
            'worst_quality' => $worstQuality,
            'worst_quality_label' => $worstQuality !== null ? ($qualityLabels[$worstQuality] ?? 'Kode ' . $worstQuality) : null,
            'facility_count' => count($facilities),
            'active_facility_count' => $activeFacilities,
            'paid_off_facility_count' => max(0, count($facilities) - $activeFacilities),
            'lender_count' => $lenderCount,
            'quality_distribution' => $qualityDistribution,
            'max_overdue_days' => $maxOverdueDays,
            'total_principal_arrears' => $totalPrincipalArrears,
            'total_interest_arrears' => $totalInterestArrears,
            'total_penalty' => $totalPenalty,
            'total_arrears' => $totalArrears,
            'effective_limit' => $effectiveLimit,
            'total_outstanding' => $totalOutstanding,
            'utilization_percent' => $utilizationPercent,
            'requested_loan_amount' => max(0, $requestedLoanAmount),
            'exposure_after_request' => $exposureAfterRequest,
            'recommendation_code' => $recommendationCode,
            'recommendation_label' => $recommendationLabel,
            'recommendation_note' => $hasGoodQuality
                ? 'Kualitas terburuk Lancar dan tidak ditemukan tunggakan pada data IDEP.'
                : 'Hasil ini adalah screening awal. Perlu verifikasi dokumen dan kebijakan kredit sebelum keputusan final.',
            'overvalue_status' => 'NOT_ASSESSABLE',
            'overvalue_label' => 'Belum dapat dinilai',
            'overvalue_note' => 'IDEP tidak memuat penghasilan dan nilai agunan. Analisa kemampuan bayar serta nilai agunan tetap dilakukan manual.',
        ];
    }

    /**
     * # COMPONENT: Membaca file IDEP tersimpan hanya dari folder upload yang diizinkan.
     * File mentah tidak dikirim ke browser; helper ini hanya dipakai untuk membangun ulang ringkasan analisis.
     */
    private function readStoredIdepContents(?string $relativePath): ?string
    {
        $relativePath = ltrim(str_replace('\\', '/', trim((string) $relativePath)), '/');
        if ($relativePath === '' || !str_starts_with($relativePath, 'uploads/idep/')) {
            return null;
        }

        $uploadRoot = realpath(__DIR__ . '/../../uploads/idep');
        $filePath = realpath(__DIR__ . '/../../' . $relativePath);
        if ($uploadRoot === false || $filePath === false) {
            return null;
        }

        $rootPrefix = rtrim($uploadRoot, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR;
        if (strncmp($filePath, $rootPrefix, strlen($rootPrefix)) !== 0) {
            return null;
        }
    ...
    /**
     * # COMPONENT: Analisis ulang IDEP lama tanpa mengakhiri response jika file historis rusak.
     */
    private function analyzeStoredIdepContents(?string $contents, int $requestedLoanAmount = 0): ?array
    {
        if ($contents === null) {
            return null;
        }

        $normalizedContents = preg_replace('/^\xEF\xBB\xBF/', '', $contents) ?? $contents;
        if (function_exists('iconv')) {
            $converted = iconv('UTF-8', 'UTF-8//IGNORE', $normalizedContents);
            if ($converted !== false) {
                $normalizedContents = $converted;
            }
        }

        try {
            $report = json_decode(trim($normalizedContents), true, 512, JSON_THROW_ON_ERROR);
        } catch (JsonException $e) {
            return null;
        }
        if (!is_array($report)) {
        ...
        return $this->analyzeIdepContent($contents, $requestedLoanAmount);
    }

    private function saveBase64Upload(
        string $base64,
        string $mimeType,
        string $prefix,
        array $allowedMime,
        string $folder = 'credit_pipeline',
        ?int $maxBytes = null
    ): array
    {
        ...
        $binary = $this->decodeBase64Payload($base64);
        if ($maxBytes !== null && strlen($binary) > $maxBytes) {
            sendResponse(400, 'Ukuran file melebihi batas yang diizinkan', null);
        }
        ...
        return [
            ...
            'type' => str_starts_with($mimeType, 'image/') ? 'IMAGE' : ($mimeType === 'text/plain' ? 'TEXT' : 'PDF'),
        ];
    }

    private function ensureCreditPipeline(
        array $prospect,
        array $user,
        ?string $note = null,
        ?int $requestedLoanAmount = null,
        ?bool $slikChecked = null,
        ?string $plannedRealizationDate = null,
        ?string $idepNumber = null
    ): int
    {
        $this->ensureColumnExists('prospect_credit_pipelines', 'requested_loan_amount', 'BIGINT UNSIGNED DEFAULT NULL AFTER `assigned_to`');
        $this->ensureColumnExists('prospect_credit_pipelines', 'slik_checked', 'TINYINT(1) DEFAULT NULL AFTER `requested_loan_amount`');
        $this->ensureColumnExists('prospect_credit_pipelines', 'planned_realization_date', 'DATE DEFAULT NULL AFTER `slik_checked`');
        $this->ensureColumnExists('prospect_credit_pipelines', 'idep_number', 'VARCHAR(50) DEFAULT NULL AFTER `planned_realization_date`');
        $this->ensureColumnExists('prospect_credit_pipelines', 'idep_file_url', 'VARCHAR(500) DEFAULT NULL AFTER `idep_number`');
        $this->ensureColumnExists('prospect_credit_pipelines', 'idep_analysis_json', 'TEXT DEFAULT NULL AFTER `idep_file_url`');
        $this->ensureColumnExists('prospect_credit_pipelines', 'realization_probability', 'TINYINT UNSIGNED DEFAULT NULL AFTER `idep_file_url`');
        ...
        if ($existing) {
            ...
            if ($requestedLoanAmount !== null && $requestedLoanAmount > 0) {
                $updates[] = 'requested_loan_amount = :amount';
                $params[':amount'] = $requestedLoanAmount;
            }
            if ($slikChecked !== null) {
                $updates[] = 'slik_checked = :slik_checked';
                $params[':slik_checked'] = $slikChecked ? 1 : 0;
            }
            if ($plannedRealizationDate !== null) {
                $updates[] = 'planned_realization_date = :planned_realization_date';
                $params[':planned_realization_date'] = $plannedRealizationDate;
            }
            if ($idepNumber !== null && $idepNumber !== '') {
                $updates[] = 'idep_number = :idep_number';
                $params[':idep_number'] = $idepNumber;
            }
            if ($updates) {
                $this->db->prepare("UPDATE prospect_credit_pipelines SET " . implode(', ', $updates) . ", updated_at = NOW() WHERE id = :id")
                    ->execute($params);
            }
        ...
        $ins = $this->db->prepare("INSERT INTO prospect_credit_pipelines
            (prospect_id, assigned_to, requested_loan_amount, slik_checked, planned_realization_date, idep_number, confirmation_at, current_stage, pipeline_status, created_by)
            VALUES (:pid, :assigned_to, :requested_loan_amount, :slik_checked, :planned_realization_date, :idep_number, :confirmation_at, 'FORMULIR', 'PROSPECT_CONFIRMED', :created_by)");
        $ins->execute([
            ':pid' => $prospect['id'],
            ...
            ':slik_checked' => $slikChecked === null ? null : ($slikChecked ? 1 : 0),
            ':planned_realization_date' => $plannedRealizationDate,
            ':idep_number' => $idepNumber,
            ':confirmation_at' => $now,
        ...
        $docStmt = $this->db->prepare("INSERT INTO prospect_credit_pipeline_documents
            ...
            VALUES (:pipeline_id, :doc_code, :doc_name, 0)");
    ...
    public function list(array $params): void
    {
        ...
        $hasSlikCheckedColumn = $this->hasPipelineColumn('slik_checked');
        $hasPlannedRealizationColumn = $this->hasPipelineColumn('planned_realization_date');

        // Base query
        $sql = "SELECT p.*, kk.nama_kantor, kk.korwil,
                    pc.id AS credit_pipeline_id,
                    pc.requested_loan_amount,
                    " . ($hasPlannedRealizationColumn ? 'pc.planned_realization_date' : 'NULL') . " AS planned_realization_date,
                    pc.current_stage AS credit_pipeline_stage,
                    pc.pipeline_status AS credit_pipeline_status,
                    DATEDIFF(COALESCE(p.closed_at, NOW()), COALESCE(pc.sla_started_at, pc.confirmation_at, p.sla_started_at, p.assigned_at, p.created_at)) AS pipeline_days
                FROM prospects p
        ...
        // --- FILTER: beberapa status sekaligus (dipakai inbox prospek) ---
        $statusIn = array_values(array_filter(array_map(
            static fn($status) => strtoupper(trim((string)$status)),
            explode(',', (string)($params['status_in'] ?? ''))
        )));
        $allowedStatuses = ['OPEN', 'FOLLOW_UP', 'SLA', 'CLOSING', 'REJECT'];
        $statusIn = array_values(array_intersect($statusIn, $allowedStatuses));
        if ($statusIn && !$filterStatus) {
            $statusPlaceholders = [];
            foreach ($statusIn as $i => $status) {
                $key = ":status_in_{$i}";
                $statusPlaceholders[] = $key;
                $binds[$key] = $status;
            }
            $sql .= " AND p.status IN (" . implode(',', $statusPlaceholders) . ")";
        }
        ...
        if ($isPipelineCredit) {
            $sql .= " AND p.prospect_type IN ('KREDIT','DEBITUR_EXISTING')
                ...
                AND p.status <> 'REJECT'";
            if ($hasSlikCheckedColumn) {
                // Pipeline lama tanpa metadata SLIK tetap terlihat sampai dimigrasikan.
                $sql .= " AND COALESCE(pc.slik_checked, 1) = 1";
            }
        }

        $pipelineStage = strtoupper(trim((string)($params['pipeline_stage'] ?? '')));
        $allowedPipelineStages = ['FORMULIR', 'PEMBERKASAN', 'SURVEY', 'ANALISA', 'KOMITE', 'CAIR', 'SELESAI'];
        if ($isPipelineCredit && in_array($pipelineStage, $allowedPipelineStages, true)) {
            $sql .= " AND pc.current_stage = :pipeline_stage";
            $binds[':pipeline_stage'] = $pipelineStage;
        }

        $pipelineStatus = strtoupper(trim((string)($params['pipeline_status'] ?? '')));
        $allowedPipelineStatuses = ['PROSPECT_CONFIRMED', 'SLA_RUNNING', 'APPROVED', 'DISBURSED'];
        if ($isPipelineCredit && in_array($pipelineStatus, $allowedPipelineStatuses, true)) {
            $sql .= " AND pc.pipeline_status = :pipeline_status";
            $binds[':pipeline_status'] = $pipelineStatus;
        }
        ...
        if (!$isMineSource && ($closingDate || $harianDate)) {
            // Pipeline mengikuti aktivitas SLA terakhir. Prospek lama yang maju tahap
            // pada periode berjalan tetap masuk walaupun created_at-nya bulan lalu.
            $periodColumn = $isPipelineCredit
                ? "COALESCE((SELECT MAX(ps.stage_started_at) FROM prospect_credit_pipeline_stages ps WHERE ps.prospect_id = p.id), pc.updated_at, pc.confirmation_at, p.created_at)"
                : 'p.created_at';
            if ($closingDate) {
                $sql .= " AND {$periodColumn} > :closing_date_limit";
                $binds[':closing_date_limit'] = $closingDate . ' 23:59:59';
            }
            if ($harianDate) {
                $sql .= " AND {$periodColumn} <= :harian_date_limit";
        ...
        // --- FILTER: segmentasi lifecycle inbox prospek ---
        $lifecycle = strtolower(trim((string)($params['lifecycle'] ?? '')));
        if (!$isMineSource && in_array($lifecycle, ['active', 'hot', 'stale'], true)) {
            $sql .= " AND p.created_at >= :lifecycle_from";
            $binds[':lifecycle_from'] = date('Y-m-d 00:00:00', strtotime('-3 months'));

            if ($lifecycle === 'hot') {
                $sql .= " AND p.status IN ('OPEN', 'FOLLOW_UP') AND p.created_at >= :lifecycle_hot_from";
                $binds[':lifecycle_hot_from'] = date('Y-m-d 00:00:00', strtotime('-1 month'));
            } elseif ($lifecycle === 'stale') {
                $sql .= " AND p.status = 'FOLLOW_UP'
                    AND p.created_at < :lifecycle_stale_to
                    AND pc.id IS NULL";
                $binds[':lifecycle_stale_to'] = date('Y-m-d 00:00:00', strtotime('-1 month'));
            } else {
                $sql .= " AND p.status IN ('OPEN', 'FOLLOW_UP')";
            }
        }
        ...
        // COMPONENT: Simpan query count sebelum ORDER/LIMIT agar subquery kantor tidak salah dipotong regex.
        $countSql = preg_replace(
            '/^SELECT.*?FROM\s+prospects\s+p/si',
        ...
        // Count total menggunakan query yang sama sebelum ORDER/LIMIT.
    ...
    public function detail(int $id): void
    {
        ...
        if ($this->isCreditProspect($prospect)) {
            ...
            if ($pipeline) {
                // # FLOW: Pipeline lama mungkin memiliki file IDEP tetapi belum menyimpan ringkasan analisis.
                // Backfill dilakukan saat detail dibuka agar analisis tetap tersedia setelah status CLOSING.
                if (empty($pipeline['idep_analysis_json']) && !empty($pipeline['idep_file_url'])) {
                    $idepContents = $this->readStoredIdepContents($pipeline['idep_file_url']);
                    $analysis = $this->analyzeStoredIdepContents(
                        $idepContents,
                        (int) ($pipeline['requested_loan_amount'] ?? 0)
                    );
                    if ($analysis !== null) {
                        $analysisJson = json_encode($analysis, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
                        $this->db->prepare("UPDATE prospect_credit_pipelines SET idep_analysis_json = :analysis, updated_at = NOW() WHERE id = :id")
                            ->execute([
                                ':analysis' => $analysisJson,
                                ':id' => $pipeline['id'],
                        ]);
                        $pipeline['idep_analysis_json'] = $analysisJson;
                    } else {
                        $pipeline['idep_analysis_error'] = $idepContents === null
                            ? 'File IDEP tidak ditemukan pada penyimpanan aplikasi.'
                            : 'File IDEP tersimpan, tetapi formatnya bukan JSON IDEP yang valid.';
                    }
                }
    ...
    public function confirmCreditInterest(array $input): void
    {
        $user = $this->getCurrentUser();
        $prospectId = (int)($input['prospect_id'] ?? 0);
        $slikRaw = $input['slik_checked'] ?? null;
        $slikChecked = is_bool($slikRaw) ? $slikRaw : match (strtoupper(trim((string) $slikRaw))) {
            '1', 'YA', 'SUDAH', 'TRUE' => true,
            '0', 'TIDAK', 'BELUM', 'FALSE' => false,
            default => null,
        };
        $requestedLoanAmount = (int)($input['requested_loan_amount'] ?? 0);
        $plannedRealizationDate = trim((string)($input['planned_realization_date'] ?? ''));
        $idepNumber = trim((string)($input['idep_number'] ?? ''));
        $idepFileBase64 = trim((string)($input['idep_file_base64'] ?? ''));
        $idepFileMimeType = strtolower(trim((string)($input['idep_file_mime_type'] ?? '')));
        ...
        if ($slikChecked === null) {
            sendResponse(400, 'Hasil SLIK wajib dipilih', null);
        }

        if ($slikChecked === false) {
            # COMPONENT: Hasil SLIK buruk menghentikan flow dan memakai mekanisme reject terpusat.
            $this->reject([
                'prospect_id' => $prospectId,
                'reject_reason' => 'SLIK tidak bagus',
                'reject_note' => 'Prospek ditolak otomatis dari konfirmasi hasil SLIK.',
            ]);
            return;
        }

        if ($requestedLoanAmount <= 0) sendResponse(400, 'Jumlah pengajuan debitur wajib diisi', null);
        if ($idepNumber === '') sendResponse(400, 'Nomor IDEP wajib diisi', null);
        if ($idepFileBase64 !== '' && $idepFileMimeType !== 'text/plain') {
            sendResponse(400, 'File IDEP harus berformat TXT', null);
        }
        if ($idepFileBase64 === '' && $idepFileMimeType !== '') {
            sendResponse(400, 'Isi file IDEP tidak ditemukan', null);
        }
        $parsedRealizationDate = DateTime::createFromFormat('!Y-m-d', $plannedRealizationDate);
        $dateErrors = DateTime::getLastErrors();
        if (!$parsedRealizationDate
            || ($dateErrors !== false && ($dateErrors['warning_count'] > 0 || $dateErrors['error_count'] > 0))
            || $parsedRealizationDate->format('Y-m-d') !== $plannedRealizationDate) {
            sendResponse(400, 'Rencana tanggal realisasi tidak valid', null);
        }

        $idepAnalysis = null;
        if ($idepFileBase64 !== '') {
            // # COMPONENT: IDEP dibaca sebelum disimpan agar file yang masuk dapat langsung dianalisis.
            $idepBinary = $this->decodeBase64Payload($idepFileBase64);
            if (strlen($idepBinary) > 2 * 1024 * 1024) {
                sendResponse(400, 'Ukuran file IDEP melebihi batas 2 MB', null);
            }
            $idepAnalysis = $this->analyzeIdepContent($idepBinary, $requestedLoanAmount);
        }

        $pipelineId = $this->ensureCreditPipeline(
            $prospect,
            $user,
            $input['note'] ?? null,
            $requestedLoanAmount,
            $slikChecked,
            $plannedRealizationDate,
            $idepNumber
        );
        if ($idepFileBase64 !== '') {
            $savedIdep = $this->saveBase64Upload($idepFileBase64, $idepFileMimeType, "idep_{$prospectId}", ['text/plain'], 'idep', 2 * 1024 * 1024);
            $this->db->prepare("UPDATE prospect_credit_pipelines SET idep_file_url = :file_url, idep_analysis_json = :analysis, updated_at = NOW() WHERE id = :id")
                ->execute([
                    ':file_url' => $savedIdep['path'],
                    ':analysis' => json_encode($idepAnalysis, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES),
                    ':id' => $pipelineId,
                ]);
        }
        if ($prospect['status'] === 'OPEN') {
    ...
    public function completeCreditDocumentation(array $input): void
    {
        ...
        # COMPONENT: SLA hanya boleh dimulai setelah modal konfirmasi membuat pipeline beserta metadata awalnya.
        $this->ensureColumnExists('prospect_credit_pipelines', 'requested_loan_amount', 'BIGINT UNSIGNED DEFAULT NULL AFTER `assigned_to`');
        $this->ensureColumnExists('prospect_credit_pipelines', 'slik_checked', 'TINYINT(1) DEFAULT NULL AFTER `requested_loan_amount`');
        $this->ensureColumnExists('prospect_credit_pipelines', 'planned_realization_date', 'DATE DEFAULT NULL AFTER `slik_checked`');
        $this->ensureColumnExists('prospect_credit_pipelines', 'idep_number', 'VARCHAR(50) DEFAULT NULL AFTER `planned_realization_date`');
        ...
        $this->ensureColumnExists('prospect_credit_pipelines', 'idep_analysis_json', 'TEXT DEFAULT NULL AFTER `idep_file_url`');
        $this->ensureColumnExists('prospect_credit_pipelines', 'realization_probability', 'TINYINT UNSIGNED DEFAULT NULL AFTER `idep_file_url`');
        $pipelineLookupStmt = $this->db->prepare("SELECT id, sla_started_at, slik_checked, idep_number FROM prospect_credit_pipelines WHERE prospect_id = :prospect_id");
        $pipelineLookupStmt->execute([':prospect_id' => $prospectId]);
        $pipelineLookup = $pipelineLookupStmt->fetch();
        if (!$pipelineLookup) {
            sendResponse(400, 'Konfirmasi Debitur Mau Lanjut harus dilakukan terlebih dahulu', null);
        }

        if ((int)($pipelineLookup['slik_checked'] ?? 0) !== 1 || trim((string)($pipelineLookup['idep_number'] ?? '')) === '') {
            sendResponse(400, 'Pipeline belum memiliki hasil SLIK bagus dan nomor IDEP', null);
        }
        $pipelineId = (int) $pipelineLookup['id'];
        $now = date('Y-m-d H:i:s');
        $deadline = date('Y-m-d H:i:s', strtotime($now . ' +14 days'));

        if (!empty($pipelineLookup['sla_started_at'])) {
            sendResponse(400, 'SLA pipeline sudah dimulai', null);
        }
        ...
        $this->db->prepare("INSERT INTO prospect_credit_pipeline_stages
            ...
            ->execute([
                ...
                ':prospect_id' => $prospectId,
                ':started_at' => $now,
                ':note' => $input['note'] ?? 'Proses SLA dimulai; dokumen dapat dilengkapi menyusul',
        ...
        if ((int)$legacyCount->fetchColumn() === 0) {
            $this->db->prepare("INSERT INTO prospect_sla_logs
                ...
                VALUES (:pid, 'VERIFIKASI_DATA', :started, 'Proses SLA dimulai; dokumen dapat dilengkapi menyusul', :by)")
        ...
        $this->addHistory($prospectId, 'SLA_STARTED', $prospect['status'], 'SLA', null, null,
            'Proses SLA dimulai tanpa menunggu upload dokumen', ['pipeline_id' => $pipelineId, 'sla_started_at' => $now]);

        sendResponse(200, 'SLA kredit dimulai. Dokumen dapat diupload menyusul.', ['pipeline_id' => $pipelineId, 'sla_started_at' => $now]);
    }
    ...
    public function uploadCreditPipelineFile(array $input): void
    {
        ...
        if ($target === 'STAGE_ATTACHMENT') {
            ...
            if (!in_array($stage, ['SURVEY', 'KOMITE'], true)) {
            ...
            if ($stage === 'SURVEY') {
                # COMPONENT: Upload susulan Survey dapat menambah foto sampai batas empat file.
                $base64Files = is_array($fileBase64) ? array_values($fileBase64) : [$fileBase64];
                $mimeFiles = is_array($mimeType) ? array_values($mimeType) : [$mimeType];
                if (count($base64Files) > 4 || count($mimeFiles) > 4 || count($base64Files) !== count($mimeFiles)) {
                    sendResponse(400, 'Foto Survey maksimal 4 file', null);
                }

                $existingUrls = json_decode((string)($stageRow['attachment_url'] ?? ''), true);
                if (!is_array($existingUrls)) {
                    $existingUrl = trim((string)($stageRow['attachment_url'] ?? ''));
                    $existingUrls = $existingUrl !== '' ? [$existingUrl] : [];
                }
                if (count($existingUrls) + count($base64Files) > 4) {
                    sendResponse(400, 'Total foto Survey maksimal 4 file', null);
                }

                $savedFiles = [];
                foreach ($base64Files as $index => $base64File) {
                    $savedFiles[] = $this->saveBase64Upload(
                        (string)$base64File,
                        (string)($mimeFiles[$index] ?? ''),
                        "stage_{$prospectId}_survey_" . ($index + 1),
                        ['image/jpeg', 'image/png'],
                        'credit_pipeline',
                        1 * 1024 * 1024
                    );
                }
                $allUrls = array_merge($existingUrls, array_column($savedFiles, 'path'));
                $saved = [
                    'path' => json_encode($allUrls, JSON_UNESCAPED_SLASHES),
                    'type' => 'IMAGE_SET',
                    'count' => count($allUrls),
                ];
            } else {
                $saved = $this->saveBase64Upload(
                    $fileBase64,
                    $mimeType,
    ...
    /**
     * # COMPONENT: Preview analisis IDEP di modal konfirmasi tanpa menyimpan file atau mengubah status.
     */
    public function analyzeIdepPreview(array $input): void
    {
        $user = $this->getCurrentUser();
        $prospectId = (int)($input['prospect_id'] ?? 0);
        $fileBase64 = trim((string)($input['file_base64'] ?? ''));
        ...
        if (!$prospect) sendResponse(404, 'Prospek tidak ditemukan', null);
        if (!$this->canAccessProspect($prospect, $user)) sendResponse(403, 'Anda tidak punya akses ke prospek ini', null);
        if (!$this->canUpdateSla($prospect, $user)) sendResponse(403, 'Hanya AO pengelola yang bisa menganalisis IDEP', null);
        if (!$this->isCreditProspect($prospect)) sendResponse(400, 'IDEP hanya untuk prospek kredit', null);
        if ($prospect['status'] === 'REJECT') sendResponse(400, 'Prospek berstatus reject dan IDEP tidak dapat dianalisis', null);

        $binary = $this->decodeBase64Payload($fileBase64);
        if (strlen($binary) > 2 * 1024 * 1024) {
            sendResponse(400, 'Ukuran file IDEP melebihi batas 2 MB', null);
        }

        sendResponse(200, 'File IDEP berhasil dianalisis', [
            'analysis' => $this->analyzeIdepContent($binary, max(0, $requestedLoanAmount)),
        ]);
    }

    /**
     * # COMPONENT: Upload IDEP dari detail pipeline dan simpan hasil screening ringkasnya.
     */
    public function uploadIdepFile(array $input): void
    {
        $user = $this->getCurrentUser();
        $prospectId = (int)($input['prospect_id'] ?? 0);
        $fileBase64 = trim((string)($input['file_base64'] ?? ''));
        ...
        if (!$prospect) sendResponse(404, 'Prospek tidak ditemukan', null);
        if (!$this->canAccessProspect($prospect, $user)) sendResponse(403, 'Anda tidak punya akses ke prospek ini', null);
        if (!$this->canUpdateSla($prospect, $user)) sendResponse(403, 'Hanya AO pengelola yang bisa upload IDEP', null);
        if (!$this->isCreditProspect($prospect)) sendResponse(400, 'IDEP hanya untuk prospek kredit', null);
        # COMPONENT: IDEP masih boleh dilengkapi atau diganti setelah closing sementara; reject tetap dikunci.
        if ($prospect['status'] === 'REJECT') sendResponse(400, 'Prospek berstatus reject dan file IDEP tidak dapat diubah', null);

        $pipelineStmt = $this->db->prepare("SELECT * FROM prospect_credit_pipelines WHERE prospect_id = :prospect_id");
        $pipelineStmt->execute([':prospect_id' => $prospectId]);
        $pipeline = $pipelineStmt->fetch();
        if (!$pipeline) sendResponse(400, 'Pipeline kredit belum dibuat', null);
        ...
        $analysis = $this->analyzeIdepContent($binary, (int)($pipeline['requested_loan_amount'] ?? 0));
        $saved = $this->saveBase64Upload($fileBase64, $mimeType, "idep_{$prospectId}", ['text/plain'], 'idep', 2 * 1024 * 1024);
        $this->db->prepare("UPDATE prospect_credit_pipelines SET
            idep_file_url = :file_url, idep_analysis_json = :analysis, updated_at = NOW()
            WHERE id = :id")
            ->execute([
                ':file_url' => $saved['path'],
                ':analysis' => json_encode($analysis, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES),
                ':id' => $pipeline['id'],
            ]);

        $this->addHistory($prospectId, 'IDEP_UPLOADED', null, null, null, null,
            'File IDEP diupload dan screening SLIK diperbarui', [
                'pipeline_id' => (int)$pipeline['id'],
                'recommendation_code' => $analysis['recommendation_code'],
                'worst_quality' => $analysis['worst_quality'],
                'total_arrears' => $analysis['total_arrears'],
            ]);

        sendResponse(200, 'File IDEP berhasil diupload dan dianalisis', [
            'file' => $saved,
            'analysis' => $analysis,
        ]);
    }
    ...
    public function addSlaLog(array $input): void
    {
        ...
        // COMPONENT: Survey boleh berjalan tanpa foto, atau menyimpan maksimal empat foto.
        $attachments = [];
        $attachment = null;
        if ($stage === 'SURVEY') {
            $base64Files = $input['attachment_base64'] ?? [];
            $mimeFiles = $input['attachment_mime'] ?? [];
            $base64Files = is_array($base64Files) ? array_values($base64Files) : ($base64Files !== '' ? [$base64Files] : []);
            $mimeFiles = is_array($mimeFiles) ? array_values($mimeFiles) : ($mimeFiles !== '' ? [$mimeFiles] : []);
            if (count($base64Files) > 4 || count($mimeFiles) > 4 || count($base64Files) !== count($mimeFiles)) {
                sendResponse(400, 'Foto Survey maksimal 4 file', null);
            }
            foreach ($base64Files as $index => $base64File) {
                $attachments[] = $this->saveBase64Upload(
                    (string)$base64File,
                    (string)($mimeFiles[$index] ?? ''),
                    "stage_{$prospectId}_survey_" . ($index + 1),
                    ['image/jpeg', 'image/png'],
        ...
        } elseif (!empty($input['attachment_base64']) && !empty($input['attachment_mime'])) {
            if ($stage === 'KOMITE') {
                ...
                sendResponse(400, 'Tahap Analisa tidak menerima upload atau foto', null);
            }
        }

        // Close previous open stage setelah lampiran lolos validasi.
        $now = date('Y-m-d H:i:s');
        $this->db->prepare("UPDATE prospect_credit_pipeline_stages SET stage_ended_at = :ended,
            duration_days = DATEDIFF(:ended2, stage_started_at)
            WHERE pipeline_id = :pipeline_id AND stage_ended_at IS NULL")
        ...
        $attachmentUrl = $stage === 'SURVEY'
            ? ($attachments ? json_encode(array_column($attachments, 'path'), JSON_UNESCAPED_SLASHES) : null)
            : ($attachment['path'] ?? null);
        $attachmentType = $stage === 'SURVEY'
            ? ($attachments ? 'IMAGE_SET' : null)
            : ($attachment['type'] ?? null);
        $attachmentUploadedAt = ($attachments || $attachment) ? $now : null;
        ...
        $ins->execute([
            ...
            ':attachment_uploaded_at' => $attachmentUploadedAt,
    ...
    public function report(array $params): void
    {
        ...
        // Optional input date range for pipeline/work-queue summaries.
        $inputDateWhere = "";
        if (!$isMineSource) {
            $inputDateFrom = $params['date_from'] ?? null;
            $inputDateTo = $params['date_to'] ?? null;
            if ($inputDateFrom) {
                $inputDateWhere .= " AND p.created_at >= :report_date_from";
                $binds[':report_date_from'] = $inputDateFrom . ' 00:00:00';
            }
            if ($inputDateTo) {
                $inputDateWhere .= " AND p.created_at <= :report_date_to";
                $binds[':report_date_to'] = $inputDateTo . ' 23:59:59';
            }
        }
        ...
        if ($statusIn) {
            $statusPlaceholders = [];
            foreach ($statusIn as $i => $status) {
                $key = ":report_status_in_{$i}";
                $statusPlaceholders[] = $key;
                $binds[$key] = $status;
            }
            $statusInWhere = " AND p.status IN (" . implode(',', $statusPlaceholders) . ")";
        }
        if ($isPipelineCredit) {
            $pipelineWhere .= " AND p.prospect_type IN ('KREDIT','DEBITUR_EXISTING')
                ...
                AND p.status <> 'REJECT'";
            if ($hasSlikCheckedColumn) {
                $pipelineWhere .= " AND COALESCE(pc.slik_checked, 1) = 1";
            }
        }
        ...
        $allowedPipelineStages = ['FORMULIR', 'PEMBERKASAN', 'SURVEY', 'ANALISA', 'KOMITE', 'CAIR', 'SELESAI'];
        if ($isPipelineCredit && in_array($pipelineStage, $allowedPipelineStages, true)) {
            $pipelineWhere .= " AND pc.current_stage = :pipeline_stage";
            $binds[':pipeline_stage'] = $pipelineStage;
        }
        ...
        $allowedPipelineStatuses = ['PROSPECT_CONFIRMED', 'SLA_RUNNING', 'APPROVED', 'DISBURSED'];
        if ($isPipelineCredit && in_array($pipelineStatus, $allowedPipelineStatuses, true)) {
            $pipelineWhere .= " AND pc.pipeline_status = :pipeline_status";
            $binds[':pipeline_status'] = $pipelineStatus;
        }
        ...
        // Periode pipeline memakai aktivitas tahap SLA terakhir, bukan created_at prospek.
        $pipelinePeriodWhere = "";
        $pipelinePeriodBinds = [];
        if ($isPipelineCredit && (($params['closing_date'] ?? null) || ($params['harian_date'] ?? null))) {
            $latestPipelineActivity = "COALESCE((SELECT MAX(ps.stage_started_at) FROM prospect_credit_pipeline_stages ps WHERE ps.prospect_id = p.id), pc.updated_at, pc.confirmation_at, p.created_at)";
            if (!empty($params['closing_date'])) {
                $pipelinePeriodWhere .= " AND {$latestPipelineActivity} > :pipeline_period_from";
                $pipelinePeriodBinds[':pipeline_period_from'] = $params['closing_date'] . ' 23:59:59';
            }
            if (!empty($params['harian_date'])) {
                $pipelinePeriodWhere .= " AND {$latestPipelineActivity} <= :pipeline_period_to";
                $pipelinePeriodBinds[':pipeline_period_to'] = $params['harian_date'] . ' 23:59:59';
            }
        }
        ...
        $lifecycle = strtolower(trim((string)($params['lifecycle'] ?? '')));
        if (!$isMineSource && in_array($lifecycle, ['active', 'hot', 'stale'], true)) {
            $lifecycleWhere .= " AND p.created_at >= :lifecycle_from";
            $binds[':lifecycle_from'] = date('Y-m-d 00:00:00', strtotime('-3 months'));

            if ($lifecycle === 'hot') {
                $lifecycleWhere .= " AND p.status IN ('OPEN', 'FOLLOW_UP') AND p.created_at >= :lifecycle_hot_from";
                $binds[':lifecycle_hot_from'] = date('Y-m-d 00:00:00', strtotime('-1 month'));
            } elseif ($lifecycle === 'stale') {
                $lifecycleWhere .= " AND p.status = 'FOLLOW_UP'
                    AND p.created_at < :lifecycle_stale_to
                    AND pc.id IS NULL";
                $binds[':lifecycle_stale_to'] = date('Y-m-d 00:00:00', strtotime('-1 month'));
            } else {
                $lifecycleWhere .= " AND p.status IN ('OPEN', 'FOLLOW_UP')";
            }
        }

        $where = "WHERE 1=1" . $accessWhere . $kantorWhere . $sourceWhere . $statusInWhere . $pipelineWhere . $lifecycleWhere . $inputDateWhere;
        $summaryWhere = $where . $pipelinePeriodWhere;
        ...
        $summaryStmt = $this->db->prepare("SELECT 
            ...
            SUM(CASE WHEN p.status = 'SLA' THEN COALESCE(pc.requested_loan_amount, 0) ELSE 0 END) AS total_pipeline_pengajuan,
            SUM(CASE WHEN p.status = 'SLA' THEN 1 ELSE 0 END) AS total_pipeline_pengajuan_noa,
            SUM(CASE WHEN p.status = 'CLOSING' THEN COALESCE(p.closing_realization_amount, 0) ELSE 0 END) AS total_pipeline_realisasi,
            SUM(CASE WHEN p.status = 'CLOSING' THEN 1 ELSE 0 END) AS total_pipeline_realisasi_noa,
            ...
            FROM prospects p {$pipelineJoin} {$summaryWhere}");
        $summaryStmt->execute(array_merge($binds, $pipelinePeriodBinds));
        ...
        $harianColumn = $isPipelineCredit
            ? "COALESCE((SELECT MAX(ps.stage_started_at) FROM prospect_credit_pipeline_stages ps WHERE ps.prospect_id = p.id), pc.updated_at, pc.confirmation_at, p.created_at)"
            : 'p.created_at';
        $harianStmt = $this->db->prepare("SELECT COUNT(*) AS jumlah 
            FROM prospects p {$pipelineJoin} {$where} AND {$harianColumn} > :hd AND {$harianColumn} <= :hd2");
        ...
        $typeStmt = $this->db->prepare("SELECT p.prospect_type, COUNT(*) AS jumlah,
            ...
            FROM prospects p {$pipelineJoin} {$summaryWhere} GROUP BY p.prospect_type");
        $typeStmt->execute(array_merge($binds, $pipelinePeriodBinds));

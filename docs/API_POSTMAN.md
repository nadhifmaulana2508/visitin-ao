...
**stage valid:** `PEMBERKASAN` | `SURVEY` | `ANALISA` | `KOMITE`

**Aturan lampiran Survey:**

- Foto Survey bersifat opsional. Array kosong berarti tahap Survey tetap boleh disimpan tanpa foto.
- Jika diisi, `attachment_base64` dan `attachment_mime` harus berupa array dengan jumlah yang sama, berisi maksimal 4 foto.
- Format foto yang diterima adalah JPEG atau PNG. Frontend mengompres foto sebelum dikirim.
- Setiap foto maksimal 1 MB setelah kompresi; backend tetap memvalidasi batas ini.
- Foto dapat dipilih dari galeri/file picker atau dijepret langsung dari kamera perangkat.
- Data lama berupa satu path lampiran tetap dapat ditampilkan; foto baru disimpan sebagai JSON array pada kolom `attachment_url` yang sudah ada.
- Tahap `ANALISA` tidak menerima upload atau foto. Tahap ini hanya menyimpan analis cabang yang dipilih.
- Tahap `KOMITE` tetap memakai satu lampiran PDF opsional dengan batas maksimal 1 MB. Frontend mencoba mengompres PDF yang lebih besar sebelum dikirim; backend tetap memvalidasi batas akhirnya.
...
## Detail Pipeline Kredit Terbaru

Entry point halaman detail pipeline adalah `/detail-pipeline/{id}`. Route `/prospek-detail/{id}` masih dipertahankan sementara sebagai kompatibilitas bookmark lama.

### Konfirmasi Debitur Mau Lanjut

```http
POST /api/?action=prospect_confirm_credit_interest
Content-Type: application/json
Authorization: Bearer {token}
```
...
```json
{
  ...
  "slik_checked": 1,
  "idep_number": "IDEP-2026-00021",
  "idep_file_base64": "data:text/plain;base64,Li4u",
  "idep_file_mime_type": "text/plain",
  "planned_realization_date": "2026-10-15"
}
```

Jika `slik_checked` bernilai `0`, prospek langsung diubah menjadi `REJECT` dengan alasan `SLIK tidak bagus` dan pipeline tidak dibuat. Jika bernilai `1`, `idep_number`, `requested_loan_amount`, dan `planned_realization_date` wajib diisi. Tanggal wajib berformat `YYYY-MM-DD`. Lampiran IDEP boleh dikosongkan; jika dikirim harus berupa `text/plain` dengan ukuran maksimal 2 MB.

### Mulai SLA Dan Upload Dokumen

```http
POST /api/?action=prospect_complete_credit_docs
Content-Type: application/json
Authorization: Bearer {token}
```

Dokumen pipeline bersifat opsional. AO dapat memulai SLA tanpa upload dokumen dan melengkapinya setelah pipeline dibuat melalui `prospect_credit_upload`.

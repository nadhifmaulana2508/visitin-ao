...
// Rentang tanggal ini dipakai sebagai filter lanjutan berdasarkan tanggal input.
$default_lifecycle_from = date('Y-m-d', strtotime('-3 months'));
$default_lifecycle_to = date('Y-m-d');
...
<style>
    ...
    /* # COMPONENT: Toolbar filter prospek satu baris di desktop dan responsif di mobile. */
    .prospek-filter-toolbar { align-items:center; }
    .prospek-filter-toolbar .filter-tabs { flex:1 1 auto; min-width:0; margin-bottom:0; }
    .prospek-search .filter-select { min-width:0; }
...
<div class="ui-filter-component" data-filter-component>
<div class="header-compact">
    <div class="d-flex justify-content-between align-items-center">
        ...
        <div class="ui-filter-header-actions">
            <button type="button" class="ui-filter-trigger" data-filter-trigger aria-controls="advanced-filter" aria-expanded="false">
...
<!-- Stats -->
<div class="ui-metric-grid" style="margin:-30px 16px 16px 16px; position:relative; z-index:10;">
    <div class="ui-metric-card ui-metric-card--open">
        <span class="ui-metric-icon" aria-hidden="true"><i class="fa-solid fa-folder-open"></i></span>
        <span class="ui-metric-content"><span class="ui-metric-value" id="s-open">0</span><span class="ui-metric-label">Open</span></span>
    </div>
    <div class="ui-metric-card ui-metric-card--follow-up">
        <span class="ui-metric-icon" aria-hidden="true"><i class="fa-solid fa-rotate"></i></span>
        <span class="ui-metric-content"><span class="ui-metric-value" id="s-fu">0</span><span class="ui-metric-label">Follow Up</span></span>
    </div>
    <div class="ui-metric-card ui-metric-card--closing">
        <span class="ui-metric-icon" aria-hidden="true"><i class="fa-solid fa-check-double"></i></span>
        <span class="ui-metric-content"><span class="ui-metric-value" id="s-closing">0</span><span class="ui-metric-label">Closing</span></span>
    </div>
    <div class="ui-metric-card ui-metric-card--reject">
        <span class="ui-metric-icon" aria-hidden="true"><i class="fa-solid fa-xmark"></i></span>
        <span class="ui-metric-content"><span class="ui-metric-value" id="s-reject">0</span><span class="ui-metric-label">Reject</span></span>
    </div>
</div>

<!-- Filters -->
<div class="filter-section">
    <div class="ui-filter-toolbar prospek-filter-toolbar">
        <!-- # COMPONENT: Filter sumber prospek yang dapat dipakai ulang bersama search. -->
        ...
        <!-- # COMPONENT: Pencarian cepat prospek. -->
        <div class="ui-filter-search prospek-search">
            <input type="text" class="ui-filter-control filter-select flex-grow-1" id="f-search" placeholder="Cari nama nasabah...">
            <button class="ui-filter-search-button btn-filter-apply" onclick="loadData()" title="Cari"><i class="fa-solid fa-magnifying-glass"></i></button>
            <!-- # COMPONENT: Toggle daftar dan rekap ditempatkan di samping pencarian. -->
            <button class="ui-view-switch ui-view-switch--search" id="btn-view-switch" type="button" title="Lihat rekap" aria-label="Lihat rekap" onclick="toggleView()"><i class="fa-solid fa-chart-bar"></i></button>
        </div>
    </div>

    <!-- Collapsible advanced filter -->
    <div id="advanced-filter" class="ui-filter-panel" data-filter-panel hidden>
        <div class="ui-filter-panel-head">
            <h6 class="ui-filter-panel-title"><i class="fa-solid fa-sliders" aria-hidden="true"></i> Filter Lanjutan</h6>
            <button type="button" class="ui-filter-close" data-filter-close aria-label="Tutup filter">
                <i class="fa-solid fa-xmark" aria-hidden="true"></i>
            </button>
        </div>
        <!-- # COMPONENT: Periode filter bersama, ditampilkan paling atas. -->
        <div class="ui-filter-grid mb-2">
            <div class="ui-filter-field">
                <label class="ui-filter-label" for="f-closing-date">Closing (M-1)</label>
                <input type="date" class="ui-filter-control filter-select" id="f-closing-date" value="<?= $default_lifecycle_from ?>">
            </div>
            <div class="ui-filter-field">
                <label class="ui-filter-label" for="f-harian-date">Harian (Actual)</label>
                <input type="date" class="ui-filter-control filter-select" id="f-harian-date" value="<?= $default_lifecycle_to ?>">
            </div>
        </div>
        <!-- # COMPONENT: Filter kantor bersama untuk Korwil dan Cabang. -->
        <div class="ui-filter-grid mb-2">
            <div class="ui-filter-field">
                <label class="ui-filter-label" for="f-korwil">Korwil</label>
                <select class="ui-filter-control filter-select" id="f-korwil">
            ...
            <div class="ui-filter-field">
                <label class="ui-filter-label" for="f-cabang">Cabang</label>
                <select class="ui-filter-control filter-select" id="f-cabang">
        ...
        <div class="ui-filter-grid">
            <div class="ui-filter-field">
                <label class="ui-filter-label" for="f-type">Jenis</label>
                <select class="ui-filter-control filter-select" id="f-type">
            ...
            <div class="ui-filter-field">
                <label class="ui-filter-label" for="f-status">Status</label>
                <select class="ui-filter-control filter-select" id="f-status">
...
<div class="list-area" id="view-list">
    <div class="ui-responsive-data" id="prospect-container" data-responsive-data>
        <div class="ui-responsive-data-cards" data-responsive-cards></div>
        <div class="ui-responsive-data-table" data-responsive-table></div>
    </div>
...
(function() {
    ...
    if (urlSource === 'pending') currentSource = 'pending';
    ...
    function reloadAfterAdvancedFilterChange() {
        currentPage = 1;
        loadData();
        if (typeof currentView !== 'undefined' && currentView === 'report') loadReport();
    }

    fKorwil.addEventListener('change', function() {
        renderCabangFilter();
        reloadAfterAdvancedFilterChange();
    });
    ['f-type', 'f-status', 'f-cabang', 'f-closing-date', 'f-harian-date'].forEach(function(id) {
        const control = document.getElementById(id);
        if (control) control.addEventListener('change', reloadAfterAdvancedFilterChange);
    });
    ...
    function appendAdvancedListFilters(params) {
        ...
        params.set('date_from', document.getElementById('f-closing-date').value);
        params.set('date_to', document.getElementById('f-harian-date').value);
    }

    // # COMPONENT: Pilih halaman detail berdasarkan alur produk prospek.
    function getProspectDetailUrl(prospect) {
        const type = String(prospect?.prospect_type || '').toUpperCase();
        const page = ['KREDIT', 'DEBITUR_EXISTING'].includes(type) ? 'detail-pipeline' : 'detail-prospek';
        return `${BASE_APP}/${page}/${encodeURIComponent(prospect.id)}`;
    }
    ...
    window.switchView = function(view) {
        currentView = view === 'report' ? 'report' : 'list';
        document.getElementById('view-list').style.display = view === 'list' ? 'block' : 'none';
        document.getElementById('view-report').style.display = view === 'report' ? 'block' : 'none';
        const switchButton = document.getElementById('btn-view-switch');
        if (switchButton) {
            const showReportIcon = currentView === 'list';
            switchButton.innerHTML = `<i class="fa-solid ${showReportIcon ? 'fa-chart-bar' : 'fa-list'}"></i>`;
            switchButton.title = showReportIcon ? 'Lihat rekap' : 'Lihat daftar';
            switchButton.setAttribute('aria-label', switchButton.title);
        }
        if (view === 'report') loadReport();
    };
    window.toggleView = function() { switchView(currentView === 'list' ? 'report' : 'list'); };
    ...
    function renderProspectTable(items) {
        const checkHeading = canDelegateProspek ? '<th class="ui-table-check" scope="col"></th>' : '';
        const heading = `${checkHeading}<th scope="col">Nasabah</th><th scope="col">Produk</th><th scope="col">Jenis Usaha</th><th scope="col">Cabang</th><th scope="col">Status</th><th scope="col">Input</th>`;
        const rows = items.map(p => {
            const typeClass = 't-' + (p.prospect_type || '').toLowerCase();
            const typeBadge = getTypeBadge(p.prospect_type);
            const statusBadge = getStatusBadge(p.status);
            const delBadge = isDelegatableProspect(p) ? '<span class="badge-delegasi del-pending">Pending</span>' : '';
            const cabang = p.nama_kantor ? `${p.kode_kantor} - ${p.nama_kantor}` : (p.kode_kantor || '-');
            const selectable = isDelegatableProspect(p);
            ...
            const checkCell = canDelegateProspek ? `<td class="ui-table-check">${check}</td>` : '';
            const nameCell = `<a class="ui-table-primary" href="${getProspectDetailUrl(p)}">${escapeHtml(p.customer_name || '-')}</a><span class="ui-table-secondary"><span class="badge-type ${typeBadge.cls}">${typeBadge.lbl}</span></span>`;
            const statusCell = `<div class="d-flex flex-wrap gap-1">${delBadge}${statusBadge}</div>`;

            return `<tr class="ui-responsive-data-row ${typeClass}" data-prospect-id="${p.id}">
                ${checkCell}
                <td>${nameCell}</td>
                <td class="ui-table-muted">${escapeHtml(p.rekomendasi_produk || '-')}</td>
                <td class="ui-table-muted">${escapeHtml(p.jenis_usaha || '-')}</td>
                <td class="ui-table-muted">${escapeHtml(cabang)}</td>
                <td>${statusCell}</td>
                <td class="ui-table-muted">${formatDate(p.created_at)}</td>
            </tr>`;
        }).join('');

        return `<table aria-label="Tabel daftar prospek">
    ...
    function renderList(items, pagination) {
        ...
        const cards = container.querySelector('[data-responsive-cards]');
        const table = container.querySelector('[data-responsive-table]');
        ...
        cards.innerHTML = items.map(p => {
            ...
            const selectable = isDelegatableProspect(p);
            const check = selectable ? `<input type="checkbox" class="bulk-check" aria-label="Pilih prospek" onclick="event.stopPropagation();" onchange="toggleBulkProspect(this, ${p.id})">` : '';
            const cardClass = `${typeClass} ${selectable ? 'selectable' : ''}`;
            return `<a href="${getProspectDetailUrl(p)}" class="prospek-card ${cardClass}" data-prospect-id="${p.id}">
                ${check}
                ...
                <div class="d-flex justify-content-between align-items-center mt-2 pt-2" style="border-top:1px solid #F4F7F6;">
                    <span class="p-meta mb-0"><i class="fa-solid fa-building"></i>${escapeHtml(cabang)}</span>
                    <span style="font-size:0.6rem;color:#94A3B8;">${formatDate(p.created_at)}</span>
                </div>
            </a>`;
        }).join('');
        table.innerHTML = renderProspectTable(items);
    ...
    function updateStats(items) {
        ...
        items.forEach(p => {
            ...
            if (p.status === 'FOLLOW_UP') counts.FOLLOW_UP++;
            if (p.status === 'SLA') counts.SLA++;
            if (p.status === 'CLOSING') counts.CLOSING++;
            if (p.status === 'REJECT') counts.REJECT++;
        });
        document.getElementById('s-open').textContent = counts.OPEN;
        document.getElementById('s-fu').textContent = counts.FOLLOW_UP + counts.SLA;
        document.getElementById('s-closing').textContent = counts.CLOSING;
        document.getElementById('s-reject').textContent = counts.REJECT;
    }

    function isDelegatableProspect(prospect) {
        return canDelegateProspek
            && String(prospect.status || '').toUpperCase() === 'OPEN'
            && String(prospect.delegation_status || '').toUpperCase() !== 'SUDAH_DIDELEGASIKAN';
    }

    async function loadSummaryStats() {
        try {
            const params = new URLSearchParams({source: currentSource === 'pending' ? 'all' : currentSource, lifecycle: 'all'});
            appendAdvancedListFilters(params);
            const response = await fetch(BASE_APP + '/api/?action=prospect_report&' + params.toString(), {credentials:'include'});
            const body = await response.json();
            const summary = body.data?.summary || {};
            document.getElementById('s-open').textContent = summary.total_open || 0;
            document.getElementById('s-fu').textContent = Number(summary.total_follow_up || 0) + Number(summary.total_sla || 0);
            document.getElementById('s-closing').textContent = summary.total_closing || 0;
            document.getElementById('s-reject').textContent = summary.total_reject || 0;
        } catch(e) {}
    }
    ...
    window.toggleBulkProspect = function(input, id) {
        const item = input.closest('.prospek-card, .ui-responsive-data-row');
    ...
    window.clearBulkSelection = function() {
        selectedProspects.clear();
        document.querySelectorAll('.bulk-check').forEach(cb => { cb.checked = false; cb.closest('.prospek-card, .ui-responsive-data-row')?.classList.remove('selected'); });
        updateBulkBar();
    };

...
if (tableExists($db, 'prospect_credit_pipelines')) {
    if (!columnExists($db, 'prospect_credit_pipelines', 'slik_checked')) {
        $db->exec("ALTER TABLE `prospect_credit_pipelines` ADD COLUMN `slik_checked` TINYINT(1) DEFAULT NULL AFTER `requested_loan_amount`");
    }
    if (!columnExists($db, 'prospect_credit_pipelines', 'planned_realization_date')) {
        $db->exec("ALTER TABLE `prospect_credit_pipelines` ADD COLUMN `planned_realization_date` DATE DEFAULT NULL AFTER `slik_checked`");
    }
    if (!columnExists($db, 'prospect_credit_pipelines', 'idep_number')) {
        $db->exec("ALTER TABLE `prospect_credit_pipelines` ADD COLUMN `idep_number` VARCHAR(50) DEFAULT NULL AFTER `planned_realization_date`");
    }
    if (!columnExists($db, 'prospect_credit_pipelines', 'idep_file_url')) {
        $db->exec("ALTER TABLE `prospect_credit_pipelines` ADD COLUMN `idep_file_url` VARCHAR(500) DEFAULT NULL AFTER `idep_number`");
    }
    if (!columnExists($db, 'prospect_credit_pipelines', 'realization_probability')) {
        $db->exec("ALTER TABLE `prospect_credit_pipelines` ADD COLUMN `realization_probability` TINYINT UNSIGNED DEFAULT NULL AFTER `idep_file_url`");
    }
}

...
switch ($action) {
    ...
    case 'prospect_idep_analyze':
        if ($request_method !== 'POST') sendResponse(405, 'Method harus POST', null);
        $token = AuthMiddleware::require();
        (new ProspectController())->analyzeIdepPreview(readJsonBody());
        break;
    ...
    case 'prospect_idep_upload':
        if ($request_method !== 'POST') sendResponse(405, 'Method harus POST', null);
        $token = AuthMiddleware::require();
        (new ProspectController())->uploadIdepFile(readJsonBody());
        break;

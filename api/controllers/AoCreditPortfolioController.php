...
class AoCreditPortfolioController
{
    ...
    private function buildPortfolioSql(array $params, array $user): array
    {
        ...
        $binds = [
            ...
            ':trx_month_current_1a' => $dates['harian_date'],
            ':trx_month_current_1b' => $dates['harian_date'],
            ':trx_month_current_2a' => $dates['harian_date'],
            ':trx_month_current_2b' => $dates['harian_date'],
        ...
        $sql = "SELECT
                ...
                trx.tgl_trx_skrg,
                COALESCE(trx.pokok_skrg, 0) AS pokok_skrg,
                COALESCE(trx.bunga_skrg, 0) AS bunga_skrg,
                COALESCE(trx.total_bayar_sekarang, 0) AS bayar_skrg,
                CASE
                    WHEN COALESCE(pt.target_pokok_awal_bulan, 0) > 0
                    THEN ROUND((COALESCE(trx.pokok_skrg, 0) / pt.target_pokok_awal_bulan) * 100, 2)
                    ELSE 0
                END AS run_off_percent,
                CASE
            ...
            LEFT JOIN (
                SELECT
                    no_rekening,
                    MAX(CASE WHEN DATE_FORMAT(tgl_trans, '%Y-%m') = DATE_FORMAT(:trx_month_current_1a, '%Y-%m') THEN tgl_trans END) AS tgl_trx_skrg,
                    SUM(CASE WHEN DATE_FORMAT(tgl_trans, '%Y-%m') = DATE_FORMAT(:trx_month_current_1b, '%Y-%m')
                        THEN COALESCE(angsuran_pokok, 0)
                        ELSE 0 END) AS pokok_skrg,
                    SUM(CASE WHEN DATE_FORMAT(tgl_trans, '%Y-%m') = DATE_FORMAT(:trx_month_current_2a, '%Y-%m')
                        THEN (COALESCE(angsuran_bunga, 0) - COALESCE(diskon_bunga, 0))
                        ELSE 0 END) AS bunga_skrg,
                    SUM(CASE WHEN DATE_FORMAT(tgl_trans, '%Y-%m') = DATE_FORMAT(:trx_month_current_2b, '%Y-%m')
    ...
    public function list(array $params): void
    {
        ...
        $summaryStmt = $this->db->prepare("SELECT
                ...
                COALESCE(SUM(pokok_skrg), 0) AS total_run_off_pokok
            FROM ({$baseSql}) x");
        ...
        sendResponse(200, 'OK', [
            ...
            'summary' => [
                ...
                'total_run_off_pokok' => (float)($summary['total_run_off_pokok'] ?? 0),
                'run_off_percent' => (float)($summary['total_target_pokok'] ?? 0) > 0
                    ? round((((float)($summary['total_run_off_pokok'] ?? 0)) / ((float)($summary['total_target_pokok'] ?? 0))) * 100, 2)
                    : 0,
            ],
            'filters' => $dates,
            'pagination' => [
